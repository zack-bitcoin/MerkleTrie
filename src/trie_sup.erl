...
init([CFG, Amount, Mode, Location]) ->
    ...
    L1 = Location ++ "data/" ++ IDS ++ "leaves.db",
    S1 = Location ++ "data/" ++ IDS ++ "stems.db",
    Children = [{A3, {dump, start_link, [A3, L1]}, permanent, 5000, supervisor, [dump_sup]},
		{A4, {dump, start_link, [A4, S1]}, permanent, 5000, supervisor, [dump_sup]},
